...
COMPANIES_FILE = os.path.join("site", "sp500_companies.json") # Use the JSON file generated by the enrichment script
...
def get_gemini_ma_prompt(text, company_name, form_type):
    ...
    return (
        ...
        "Direct Quotes:\n"
        "- [A bulleted list of verbatim quotes supporting your findings. Ensure these quotes are clean of any XML/XBRL tags or other non-human readable formatting.]\n\n"
...
def main():
    ...
    if newly_analyzed_reports:
        logging.info(f"Writing {len(newly_analyzed_reports)} new reports to Firestore...")
        for report in newly_analyzed_reports:
            try:
                # Use a sanitized report_path as a unique ID to prevent duplicates
                doc_id = report['report_path'].replace('/', '_')
                db.collection('filings').document(doc_id).set(report)
                logging.info(f"Wrote report {doc_id} to Firestore.")
            except Exception as e:
                logging.error(f"Failed to write report {report['report_path']} to Firestore: {e}")
